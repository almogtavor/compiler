...
public class AstStmtReturn extends AstStmt {
    ...
    public void SemantMe() {
        ...
        if (retType == null) throw new SemanticException(lineNumber);
        Type expType = null;
        if (exp != null) expType = exp.SemantMe();
        if (retType == TypeVoid.getInstance()) {
            if (exp != null) throw new SemanticException(lineNumber);
            return;
        }
        if (exp == null) return;
    ...
    public Temp IRme() {
        if (exp != null) {
            Temp val = exp.IRme();
            Ir.getInstance().AddIrCommand(new IrCommandReturn(val));
        } else {
